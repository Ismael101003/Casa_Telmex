<?php
/**
 * API para guardar usuario completo con todos los campos adicionales
 * Usa conexión centralizada
 */
...
try {
    ...
    // Si es derechohabiente, también requiere cédula de afiliación
    ...
    // Verificar si el usuario ya existe (por CURP si se proporciona)
    $usuarioExistente = null;
    if (!empty($datos['curp'])) {
        $resultado = $conexion->consultar("SELECT id_usuario FROM usuarios WHERE curp = ? LIMIT 1", [$datos['curp']]);
        $usuarioExistente = !empty($resultado) ? $resultado[0] : null;
    }
    ...
    if ($usuarioExistente) {
        ...
        $nuevoId = $conexion->insertar($sql, $params);
